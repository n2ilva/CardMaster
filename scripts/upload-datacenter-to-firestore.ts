...
/**
 * Upload do catálogo do DataCenter Builder para o Firestore.
 *
 * Lê `data/data-center/datacenterbuild.json` e grava em:
 *   collection: `datacenter_catalog`
 *   document : `main`
 *
...
const COLLECTION_NAME =
  process.env.DATACENTER_COLLECTION ?? "datacenter_catalog";
const DOC_ID = process.env.DATACENTER_DOC_ID ?? "main";
const PROJECT_ID = process.env.FIREBASE_PROJECT_ID || "cardmaster-934d5";
const JSON_PATH =
  process.env.DATACENTER_JSON_PATH ?? "data/data-center/datacenterbuild.json";
...
async function loadCatalog(): Promise<Record<string, unknown>> {
  ...
  const levels = Array.isArray((data as { levels?: unknown[] }).levels)
    ? ((data as { levels: unknown[] }).levels as unknown[])
    : [];
  const cables = Array.isArray(
    (data as { cable_types?: unknown[] }).cable_types,
  )
    ? ((data as { cable_types: unknown[] }).cable_types as unknown[])
    : [];
  console.log(
    `Carregado ${JSON_PATH} — ${levels.length} cenários, ${cables.length} tipos de cabo`,
  );
  return data;
}
